...
class LinearRegression:
    ...
    def fit(self,alpha,numOfSteps):
        ...
        for i in range(len(self.Samples)): 
            t = self.getTheta1()*self.Samples[i] + self.getTheta0() 
            y1.append(t)

        plt.plot(self.Samples, self.labels, '-r')
        plt.plot(self.Samples,y1, '-b')
        title = "Iteration "+ str(self.getIteration()) +"-- hypothesis: "+ str(self.__str__()) +" -- Error:" + str(cost)
        plt.title(title)
        plt.grid()
#         plt.show()
